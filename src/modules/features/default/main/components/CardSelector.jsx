...
import { getGameCardsByEvent, getGameCardsByEventPaginated, checkCardAvailability, clearCardsPaginationCache } from 'modules/features/admin/cards';
...
const CardSelector = () => {
  ...
  useEffect(() => {
    ...
    setTermsAccepted(checkTermsAccepted());
  ...
  const taxes = selectedItems.length > 0 ? calculateTaxes(subtotal).tax + (selectedItems.length * 0.05 + selectedItems.length * 0.15) : 0; // IVA + cargo adicional por cartilla
