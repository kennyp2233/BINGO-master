  ...
  useEffect(() => {
    onAuthStateChanged(authentication, async (user) => {
      if (user) {
        setUserId(user.uid);
        try {
          // Usar el servicio de tickets para obtener las cartillas del usuario
          const userCards = await ticketsService.getGameCardsByUserEvent(user.uid, id);
          setCards(userCards);
        } catch (error) {
          console.error('Error obteniendo cartillas del usuario:', error);
          setCards([]);
        }
      }
    });
